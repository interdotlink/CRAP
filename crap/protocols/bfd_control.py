...
class BfdControl(BaseHeader):
    def __init__(self: BfdControl) -> None:
        self.data = (
            IP(
            ...
            / UDP(
                dport=Settings.BFD_CONTROL_DST_PORT,
                sport=Settings.BFD_CONTROL_SRC_PORT,
            )
            / BFD(sta="Init")
        )
        ...
        self.set_filter(
            ...
            f" and udp"
            f" and dst port {Settings.BFD_CONTROL_DST_PORT}"
            f" and src port {Settings.BFD_CONTROL_SRC_PORT}"
        )

...
from scapy.contrib.bfd import BFD  # type: ignore
...
class BfdMicro(BaseHeader):
    def __init__(self: BfdMicro) -> None:
        self.data = (
            IP(
            ...
            / UDP(
                dport=Settings.BFD_MICRO_DST_PORT,
                sport=Settings.BFD_MICRO_SRC_PORT,
            )
            / BFD(sta="Init")
        )
        ...
        self.set_filter(
            ...
            f" and udp"
            f" and dst port {Settings.BFD_MICRO_DST_PORT}"
            f" and src port {Settings.BFD_MICRO_SRC_PORT}"
        )

        build_l2(
            dst=Settings.MAC_BDF_MICRO,
            eth2=True,
            etype=ETH_P_IP,
            header=self,
